	...
	t_info *info)
{
	if (((*carriage)->arg_types[first] == 2 && \
		(info->operations[first][(*carriage)->statement_code - 1] == 2 || \
		...
		(info->operations[first][(*carriage)->statement_code - 1] == 3 || \
...
static int	if_arg_one(u_int32_t first, t_carriage **carriage, \
	t_info *info)
{
	if ((*carriage)->arg_types[first] == 1 && (info->operations[first]\
	...
	u_int8_t core[MEM_SIZE])
{
	...
	if (info->operations[first][(*carriage)->statement_code - 1] == 0)
		return (0);
	if (if_arg_one(first, carriage, info) == TRUE)
	{
	...
	else if (if_arg_two_three(first, carriage, info) == TRUE)
	{
		if ((*carriage)->arg_types[first] == 2)
			i = info->operations[SIZE][(*carriage)->statement_code - 1];
		else
			i = 2;
